...
Version 2 (2026-09-06) replaces the first release entirely. The first release could be regenerated from the public generator code because its seed was published and its case identifiers encoded the generator index; every identifier, random draw and diagnostic token in version 2 is derived from a secret that is not published, and no version-1 case survives.

The raw upload contains 900 independent cases in 45 grouping families of 20 cases each. Every case has 22 anonymous components (`cmp_00` to `cmp_21`), six anonymous environments (`env_00` to `env_05`), 96 historical probes, and 64 private counterfactual probes. There are 86,400 historical probe rows in total.
...
| File | Contents |
|---|---|
| `probes.csv` | 86,400 historical probe records, the primary observation table |
...
|---|---:|---|---|
| public `train.csv` | 720 | `case_id` | `case_id`, `family_id`, and `probes_json`, a JSON array of that case's 96 historical probes, each with `probe_id`, `environment_id`, `enabled_components`, `outcome` and `failure_trace` |
...
| private `answers.csv` | 180 | `case_id` | `prediction_json`, holding `{"graph": ..., "counterfactuals": [...]}`: the hidden graph and the 64 private counterfactual probes; the same columns as `sample_submission.csv` |
...
- **Uniform case shape.** Every case has exactly 22 components, six environments, 96 historical probes and the same inventories. Solutions are never tested on larger or ragged systems.
